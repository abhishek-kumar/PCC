...
import classifiers.EPCC_Random
from classifiers.EPCC_Random import *
import pdb
import numpy
import traceback
import sys
...
def processDataset(datafile_train, datafile_test, resultfile_train, resultfile_test, p, k):
		...
		for b in [15]: 
			...
			logf.info("\tSearch Beam size " + str(b) + "; Inference Beam Size " + str(15) + " Test Set Hamming Loss: " + str(hl))
			logf.info("\tSearch Beam size " + str(b) + "; Inference Beam Size " + str(15) + " Test Set 0-1 Loss: " + str(sl))
			logf.info("\tSearch Beam size " + str(b) + "; Inference Beam Size " + str(15) + " Test Set Rank Loss: " + str(rl))
			print >>outf_test, str(b) + "\t" + str(15) + "\t" + str(hl) + "\t" + str(sl) + "\t" + str(rl)
...
if __name__ == "__main__":
		...
		logging.basicConfig(filename="./output/EPCCRandomClassifier.log", level=logging.DEBUG)
		logf = logging.getLogger(__name__)
		...
		try:
			...
			#processDataset("../../data/scene/scene-train.csv", "../../data/scene/scene-test.csv", "./output/scene-train-pcc.txt", "./output/scene-test-epccrandom.txt", 294, 6)
			#logf.info("Finished processing the scene dataset")
			
			#logf.info("Started processing the yeast dataset")
			#processDataset("../../data/yeast/yeast-train.csv", "../../data/yeast/yeast-test.csv", "./output/yeast-train-pcc.txt", "./output/yeast-test-epccrandom.txt", 103, 14)
			#logf.info("Finished processing the yeast dataset")

			#logf.info("Started processing the medical dataset")
			#processDataset("../../data/medical/medical-train.csv", "../../data/medical/medical-test.csv", "./output/medical-train-pcc.txt", "./output/medical-test-epccrandom.txt", 1449, 45)
			#logf.info("Finished processing the medical dataset")

			#logf.info("Started processing the genbase dataset")
			#processDataset("../../data/genbase/genbase-train.csv", "../../data/genbase/genbase-test.csv", "./output/genbase-train-pcc.txt", "./output/genbase-test-epccrandom.txt", 1186,27)
			#logf.info("Finished processing the genbase dataset")

			#logf.info("Started processing the enron dataset")
			#processDataset("../../data/enron/enron-train.csv", "../../data/enron/enron-test.csv", "./output/enron-train-pcc.txt", "./output/enron-test-epccrandom.txt", 1001,53)
			#logf.info("Finished processing the enron dataset")

			logf.info("Started processing the emotions dataset")
			processDataset("../../data/emotions/emotions-train.csv", "../../data/emotions/emotions-test.csv", "./output/emotions-train-pcc.txt", "./output/emotions-test-epccrandom.txt", 72,6)
