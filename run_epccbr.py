...
import classifiers.EPCC_BR 
from classifiers.EPCC_BR import *
import pdb
import numpy
import traceback
import sys
...
if __name__ == "__main__":
		...
		logging.basicConfig(filename="./output/EPCCBRClassifier.log", level=logging.DEBUG)
		logf = logging.getLogger(__name__)
		...
		try:
			...
			processDataset("../../data/scene/scene-train.csv", "../../data/scene/scene-test.csv", "./output/scene-train-epccbr.txt", "./output/scene-test-epccbr.txt", 294, 6)
			logf.info("Finished processing the scene dataset")
			
			logf.info("Started processing the yeast dataset")
			processDataset("../../data/yeast/yeast-train.csv", "../../data/yeast/yeast-test.csv", "./output/yeast-train-epccbr.txt", "./output/yeast-test-epccbr.txt", 103, 14)
			logf.info("Finished processing the yeast dataset")

			#logf.info("Started processing the medical dataset")
			#processDataset("../../data/medical/medical-train.csv", "../../data/medical/medical-test.csv", "./output/medical-train-epccbr.txt", "./output/medical-test-epccbr.txt", 1449, 45)
			#logf.info("Finished processing the medical dataset")

			#logf.info("Started processing the genbase dataset")
			#processDataset("../../data/genbase/genbase-train.csv", "../../data/genbase/genbase-test.csv", "./output/genbase-train-epccbr.txt", "./output/genbase-test-epccbr.txt", 1186,27)
			#logf.info("Finished processing the genbase dataset")

			#logf.info("Started processing the enron dataset")
			#processDataset("../../data/enron/enron-train.csv", "../../data/enron/enron-test.csv", "./output/enron-train-epccbr.txt", "./output/enron-test-epccbr.txt", 1001,53)
			#logf.info("Finished processing the enron dataset")

			logf.info("Started processing the emotions dataset")
			processDataset("../../data/emotions/emotions-train.csv", "../../data/emotions/emotions-test.csv", "./output/emotions-train-epccbr.txt", "./output/emotions-test-epccbr.txt", 72,6)
