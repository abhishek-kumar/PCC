...
class EPCCRandomClassifier:
  ...
  def fit(self, X, Y):
    ...
    ordering = range(k) # Original ordering
    permutations = set([tuple(ordering)]) 
    while len(permutations) < b:
      random.shuffle(ordering)
      permutations.add(tuple(ordering))
      #print "\tAdded: ",ordering
    self.ensemblePermutations = [list(ex) for ex in permutations]
    print "Random ensemble has these random orderings: ",self.ensemblePermutations
    self.ensembleClf = []
    for i in range(b):
