...
class EPCCBRClassifier:
  ...
  def fit(self, X, Y):
    ...
    # Classifier #1: PCC_15^15
    #clf = FixedPCCClassifier(b=15)
    #self.ensemblePermutations = [beam[0].tagsSoFar]
    #clf.fit(X, reorder(Y, self.ensemblePermutations[0]))
    #self.ensembleClf.append(clf)

    # Classifier #2: BR
    clf2 = BinaryRelevanceClassifier()
    clf2.fit(X, Y)
    self.ensemblePermutations.append(range(k)) # For BR
    self.ensembleClf.append(clf2);
...
EPCCBRClassifier.setPCCScore = updateKTAScore
