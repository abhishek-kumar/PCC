...
class FixedPCCEAClassifier(LinearModel):
    ...
    def fit(self, x, y):
      ...
      for l in arange(k):
        ...
        if len(self.tagClassifiersOld) > l:
          ...
          logf.info("\t\tFeature space has dimensionality " + str(len(xplus[0])))
          
          # Grid search for regularization weights using 10-fold stratified CV
          parameters = {'C':[2**i for i in range(-12,12,2)]} # Exhaustive search 
    ...
    def predictNode(self, T):
      ...
      epsilon = log(self.b)
      logf.info("Epsilon (non log) = " + str(self.b))
      ...
      for tag in range(2):
        ...
        if curNode.score >= epsilon:
          Q.append(curNode)
          QScores.append(curNode.score)
          newlyAdded += 1
      if newlyAdded == 0:
        K.append(InferenceNode()) # root
        KScores.append(-1) # doesnt matter, will be ignored
      else:
        #sort nodes in order of decreasing score
        selectedNodes = argsort(QScores[:])[::-1]
        snIter = iter(selectedNodes)
        QNew = [Q[snIter.next()] for count in range(len(selectedNodes))]
        Q = QNew
        QScores = [nodei.score for nodei in Q]

      while len(Q) != 0:
        logf.info("Q scores: " + str(QScores))
        node = Q.pop(0); QScores.pop(0)
        if len(node.tagsSoFar) == k:
          v = node
          K = []; KScores = []
          break
        xplus = hstack((T, node.tagsSoFar))
        pr = self.tagClassifiers[len(node.tagsSoFar)].predict_proba( [xplus] )[0]
        pr = pCorrect(pr)
        newlyAdded = 0
        for tag in range(2):
          curNode = InferenceNode()
          curNode.makeChildOf(node, tag, pr[1])
          curNode.score += log(pr[tag])
          if curNode.score >= epsilon:
            Q.append(curNode)
            QScores.append(curNode.score)
      ...
      while len(K) != 0:
        logf.info("KScores = " + str(KScores))
        vprime = K.pop(0); KScores.pop(0)
        while len(vprime.tagsSoFar) < k:
          if len(vprime.tagsSoFar) > 0:
            xplus = hstack((T, vprime.tagsSoFar))
          else:
            xplus = T
          pr = self.tagClassifiers[len(vprime.tagsSoFar)].predict_proba( [xplus] )[0]
          pr = pCorrect(pr)
          lc = InferenceNode(); rc = InferenceNode()
          lc.makeChildOf(vprime, 0, pr[1]); rc.makeChildOf(vprime, 1, pr[1])
          lc.score += log(pr[0]); rc.score += log(pr[1])
          if (lc.score >= rc.score):
            vprime = lc
          else:
            vprime = rc
        if vprime.score >= epsilon:
          v = vprime
          epsilon = vprime.score
